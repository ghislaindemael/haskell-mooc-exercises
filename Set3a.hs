...
firstHalf str
    | l == 1 = str
    | l `mod` 2 == 0 = take (l `div` 2) str
    | otherwise = take ((l `div` 2) + 1) str
        where l = (length str)
...
capitalize str = unwords (map capitalizeWord (words str))
  where
    capitalizeWord [] = []
...
powers :: Int -> Int -> [Int]
powers k max = takeWhile (<= max) [k^n | n <- [0..]] -- Taking advantage of lazy
-- and because list comprehension is beautiful
...
joinToLength len arr = (filter (\x -> length x == len) combined)
    where combined = [a ++ b | a <- arr, b <- arr]
...
interpreter commands = reverse (interpret commands (0, 0) [])
  where
    interpret :: [String] -> (Int, Int) -> [String] -> [String]
    interpret [] (x, y) output = output
    interpret (cmd:cmds) (x, y) output =
      case cmd of
        "up" -> interpret cmds (x, y + 1) output
        "down" -> interpret cmds (x, y - 1) output
        "left" -> interpret cmds (x - 1, y) output
        "right" -> interpret cmds (x + 1, y) output
        "printX" -> interpret cmds (x, y) (show x : output)
        "printY" -> interpret cmds (x, y) (show y : output)
        _ -> interpret cmds (x, y) output
