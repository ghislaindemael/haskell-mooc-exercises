...
path maze place1 place2 = place2 `elem` evalState (visit maze place1 >> get) []
...
allSums :: [Int] -> [Int]
allSums xs = do
  choices <- sequence $ replicate (length xs) [True, False]
  let selected = [x | (x, include) <- zip xs choices, include]
  return $ sum selected
...
mkCounter = do
  ...
  let inc = modifyIORef counterRef (+1)

  let get = readIORef counterRef
