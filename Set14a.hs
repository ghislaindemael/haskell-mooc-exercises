...
import qualified GHC.IO.Encoding as Data
...
shout :: T.Text -> T.Text
shout input =
  let wordsList = T.words input
      processedWords = zipWith processWord [0 ..] wordsList
   in T.unwords processedWords
  where
    processWord :: Int -> T.Text -> T.Text
    processWord index word
      | even index = T.toUpper word
      | otherwise = word
...
longestRepeat input
  | T.null input = 0 -- Handle the empty case
  | otherwise = findLongest (T.unpack input) Nothing 0 0
  where
    findLongest :: [Char] -> Maybe Char -> Int -> Int -> Int
    findLongest [] _ currentLength maxLength = max currentLength maxLength
    findLongest (x : xs) prevChar currentLength maxLength
      | Just x == prevChar = findLongest xs prevChar (currentLength + 1) maxLength
      | otherwise = findLongest xs (Just x) 1 (max currentLength maxLength)
...
takeStrict n text = T.concat . TL.toChunks $ TL.take n text
...
byteRange bs
  ...
  where
    (minByte, maxByte) = B.foldl' (\(mn, mx) b -> (min mn b, max mx b)) (255, 0) bs
