...
treeMax Empty = 0
treeMax (Node val trA trB) = max val (max (treeMax trA) (treeMax trB))
...
isOrdered (Node a left right) = (allValues (<a) left) && (allValues (>a) right) && isOrdered left && isOrdered right
...
walk (StepL:xs) (Node a left right) = walk xs left
walk (StepR:xs) (Node a left right) = walk xs right
...
set _ _ Empty = Empty
set [] val (Node a left right) = (Node val left right)
set (StepL:xs) val (Node a left right) = Node a (set xs val left) right
set (StepR:xs) val (Node a left right) = Node a left (set xs val right)
...
contains val (Node a left right)
    | val == a = True
...
search val (Node a left right)
    | val == a = Just []
    | contains val left = Just (StepL : search' val left)
    | contains val right = Just (StepR : search' val right)
    | otherwise = Nothing
  where
    search' val' Empty = []
    search' val' (Node a' left' right')
        | val' == a' = []
        | contains val' left' = StepL : search' val' left'
        | contains val' right' = StepR : search' val' right'
        | otherwise = []
