...
sumIsLength input output = length input === sum' output
  where
    sum' [] = 0
    sum' ((_, x) : xs) = x + sum' xs
...
outputInInput input output =
  forAll (elements output) $ \(x, n) -> length (filter (== x) input) == n
...
frequenciesProp freq (NonEmpty input) =
  conjoin [ sumIsLength input (freq input),
...
instance Arbitrary Arg where
  arbitrary = oneof [
      fmap Number (choose (0, 10)),
      fmap Variable (elements "abcxyz")
    ]
