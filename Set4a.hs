...
distinct [] = True
distinct [x] = True
distinct (x:xs) = (allTrue [x /= y | y <- xs]) && distinct xs
...
maximum (x:xs)
    | x > max = x
    | otherwise = max
        where max = Set4a.maximum xs


rangeOf :: (Ord a, Num a) => [a] -> a
rangeOf lst = (Set4a.maximum lst) - (Set4a.minimum lst)
...
smallestList :: Ord a => [[a]] -> [a]
smallestList lst = head (filter (\x -> head x == smallestHead) lst)
    where smallestHead = Set4a.minimum (map head lst)

longest :: Ord a => [[a]] -> [a]
longest lst = smallestList (filter (\x -> length x == maxLen) lst)
    where maxLen = Set4a.maximum (map length lst)
...
incrementKey key lst = [if (k == key) then (k, v + 1) else (k, v)| (k, v) <- lst]
...
winner scores player1 player2
    ...
    | otherwise = player2
    where
        score1 = maybe 0 id (Map.lookup player1 scores)
        score2 = maybe 0 id (Map.lookup player2 scores)
...
transfer from to amount bank =
    case (Map.lookup from bank, Map.lookup to bank) of
        (Nothing, _) -> bank
        (_, Nothing) -> bank
        (Just senderBalance, Just receiverBalance) ->
            if amount <= 0 then
                bank
            else
                if senderBalance < amount then
                    ...
                    Map.insert from (senderBalance - amount) $
                    Map.insert to (receiverBalance + amount) bank
...
maxIndex arr = head ( filter ((== maxElem) . (arr !)) (indices arr))
  where
    maxElem = Set4a.maximum (elems arr)
