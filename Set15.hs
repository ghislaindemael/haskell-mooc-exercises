...
calculator op num = liftA2 applyOp (readMaybe num) (parseOp op)
  where
...
validateDiv a b = fmap (div a) valid
  where
    valid = check (b /= 0) "Division by zero!" b
...
validateAddress streetName streetNumber postCode = liftA3 Address checkedName checkedNum checkedPost
  where
    ...
    checkedPost = check (and (map isDigit postCode ++ [length postCode == 5])) "Invalid postcode" postCode
...
twoPersons name1 age1 employed1 name2 age2 employed2 =
  liftA2 (\(n1, a1, e1) (n2, a2, e2) -> [Person n1 a1 e1, Person n2 a2 e2])
         (liftA3 (,,) name1 age1 employed1)
         (liftA3 (,,) name2 age2 employed2)
...
parseBool a = check (a == "True" || a == "False") "Not a Bool" (Left (a == "True"))

parseInt :: String -> Validation (Either Bool Int)
parseInt a = check (isJust (readMaybe a :: Maybe Int)) "Not an Int" (Right (read a :: Int))
...
normalizePhone a = checkLength (removeSpace a) *> checkChar (removeSpace a)

removeSpace :: String -> String
removeSpace = filter (not . isSpace)

checkLength :: String -> Validation String
checkLength a = check (length a <= 10) "Too long" a

checkChar :: String -> Validation String
checkChar = traverse checkC
  where
    checkC x = check (isDigit x) ("Invalid character: " ++ [x]) x
...
parseExpression a = if length (words a) /= 3 then invalid ("Invalid expression: " ++ a) else liftA3 createExpress checkOp checkOperand1 checkOperand2
  where
    checkOp = check (getOp 1 a == "+" || getOp 1 a == "-") ("Unknown operator: " ++ getOp 1 a) (getOp 1 a)
    checkOperand1 = checkNum (getOp 0 a) <|> checkVar (getOp 0 a)
    checkOperand2 = checkNum (getOp 2 a) <|> checkVar (getOp 2 a)
    createExpress op a1 a2 = if op == "+" then Plus a1 a2 else Minus a1 a2

getOp :: Int -> String -> String
getOp n s = words s !! n

checkNum :: String -> Validation Arg
checkNum a = check (isJust (readMaybe a :: Maybe Int)) ("Invalid number: " ++ a) (Number (read a :: Int))
